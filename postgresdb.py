class PostgresDb:
    ...
    def __init__(self,  dbrunsql, dbserver, dbadmindb, dbname, dbadmin, dbadminpw):
        ...
        self.owner_pw = self.pw_gen(16)
        self.user_pw = self.pw_gen(16)
        self.sql = list()
        self.generate_sql()
    ...
    def generate_sql(self):
        ...
        self.sql.append(('create', 'CREATE USER {} WITH LOGIN CREATEDB PASSWORD \'{}\';'.format(self.owner, self.owner_pw),
                    'drop user {};'.format(self.owner)))
        self.sql.append(('create', 'CREATE USER {} WITH LOGIN PASSWORD \'{}\';'.format(self.user, self.user_pw),
                    'drop user {};'.format(self.user)))
        self.sql.append(('create', 'CREATE ROLE {}_reader;'.format(self.dbName),
                    'drop role {}_reader;'.format(self.dbName)))
        self.sql.append(('create', 'CREATE ROLE {}_writer;'.format(self.dbName),
                    'drop role {}_writer;'.format(self.dbName)))
        self.sql.append(('create', 'GRANT {} to {};'.format(self.owner, self.dbAdmin),
                    'revoke {} from {};'.format(self.owner, self.dbAdmin)))
        self.sql.append(('create', 'CREATE DATABASE {} WITH OWNER = {};'.format(self.dbName, self.owner),
                    'drop database {};'.format(self.dbname)))

        self.sql.append(('grant', '\connect {}'.format(self.dbName), None))
        self.sql.append(('grant', 'GRANT {}_reader to {};'.format(self.dbName, self.user),
                    'revoke {}_reader from {};'.format(self.dbName, self.user)))
        self.sql.append(('grant', 'GRANT {}_writer to {};'.format(self.dbName, self.user),
                    'revoke {}_writer from {};'.format(self.dbName, self.user)))
        self.sql.append(('grant', 'grant select on all tables in schema public to {}_reader;'.format(self.dbName),
                    'revoke select on all tables in schema public from {}_reader;'.format(self.dbName)))
        self.sql.append(
            (
            'grant', 'grant select, insert, update, delete on all tables in schema public to {}_writer;'.format(self.dbName),
